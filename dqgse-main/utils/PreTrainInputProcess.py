...
class TrainInputProcess:
    ...
    def get_text_dataset(self):
        # 直接指定读取debug.txt文件
        text_path = os.path.join(self.data_text_dir, "mvsa.txt")
        sentence_l = []  # 存储句子
        image_l = []  # 存储对应的image_id
        ...
        with open(text_path, 'r', encoding="utf-8") as f:
            ...
            for line in f:
                ...
                if line.startswith("IMGID:"):
                    # 如果已有句子和image_id，先添加到列表
                    if current_sentence and current_image_id:
                        sentence_l.append(current_sentence)
                        image_l.append(current_image_id)
                    # 更新当前image_id，重置句子
                    current_image_id = line.split(":")[1]
                    current_sentence = []
                else:
                    # 非IMGID行，提取单词（每行第一个元素为单词）
                    parts = line.split()
                    if parts:
        ...
        # 将结果存入data_dict（使用'debug'作为数据集类型）
        self.data_dict['mvsa'] = (sentence_l, image_l)
    ...
    def generate_dualc_input(self):
        for dataset_type in self.dataset_types:
            sentence_l, image_l= self.data_dict[dataset_type]
            images = []
            noun_masks=[]

            for image_path in image_l:
                image_file_path = os.path.join(self.data_image_dir, image_path+'.jpg')
                image = Image.open(image_file_path)
                image = image.convert('RGB')
                images.append(image)
    ...
    def create_dependency_matric(self, image_id):
        ...
        dependency_matrix_path = os.path.join('../../TextSpaCy/syn_pos_dtca', image_pt)
    ...
    def create_noun_mask(self, image_id):
        # 构建矩阵文件路径（替换扩展名）
        image_pt = image_id.split('.')[0] + '.npy'
        noun_mask_path = os.path.join('../../TextSpaCy/mvsa_noun_masks', image_pt)
...
def main():
    ...
    parser.add_argument('--dataset_type', type=str, default='mvsa', nargs='?', help='display an string')
    ...
    # inputs output dir
    parser.add_argument('--output_dir', type=str, default='datasets/pretrain', nargs='?', help='display an string')
    ...
    data_text_dir = None
    data_image_dir = None
    if dataset_type == 'mvsa':
        data_text_dir = '../../TextSpaCy'
        data_image_dir = '../../lqmasa/MVSA/data'
