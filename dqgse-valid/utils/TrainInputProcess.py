...
from transformers import AutoTokenizer, GPT2TokenizerFast, ViTImageProcessor, VisionEncoderDecoderModel, BertModel, \
    RobertaModel, BlipForConditionalGeneration, BlipProcessor, CLIPProcessor, AutoProcessor,Wav2Vec2FeatureExtractor,WavLMModel
...
class TrainInputProcess:
    def __init__(self,
        ...
        #self.audio_feature_extractor = Wav2Vec2FeatureExtractor.from_pretrained("./WavLM")
    ...
    def generate_dualc_input(self):
        for dataset_type in self.dataset_types:
            ...
            #audios=[]
            #audios_attention=[]
            with open('datasets/twitter2017/output.json', 'r') as f:
                json_data = f.read()
                image_anps=json.loads(json_data)
            for image_path in image_l:
                image_file_path = os.path.join(self.data_image_dir, image_path)
                image = Image.open(image_file_path)
                image = image.convert('RGB')
...
#                 audio_file_path=os.path.join('../audio_features',image_id+'.pt')
#                 #speech, sample_rate = librosa.load(audio_file_path, sr=16000)
#                 data = torch.load(audio_file_path)
#                 audio_feat = data['input_values'].squeeze(0)
#                 audio_mask = data['attention_mask'].squeeze(0) 
#                 audios.append(audio_feat)
#                 audios_attention.append(audio_mask)
                dependency_matric=self.create_dependency_matric(image_id)
                dependency_matrics.append(dependency_matric)
                noun_mask=self.create_noun_mask(image_id)
                noun_masks.append(noun_mask)
                if dataset_type == 'train':
                    image_anp=image_anps.get(image_path)
                    imagelabel_value = [0] * 2089
                    image_anp = dict(sorted(image_anp.items()))
                    for i, (k, v) in enumerate(image_anp.items()):
                        imagelabel_value[i] = v
                    imagelabel_value=torch.tensor(imagelabel_value,dtype=torch.float)
                    image_labels.append(imagelabel_value)
                # image_pt = image_id + '.pt'
                # features_path = os.path.join('data/region_feature', image_pt)
                # if os.path.exists(features_path):
                #     features = torch.load(features_path)
                # else:
                #     # 假设特征向量维度是 2048，根据实际情况调整
                #     features = torch.zeros(36, 768)
...
#                 audios,  # 传入批量音频（列表形式的numpy数组）
#                 sampling_rate=self.audio_feature_extractor.sampling_rate,
#                 return_tensors="pt",  # 返回PyTorch tensor
#                 padding=True,  # 自动填充到批次中最长音频的长度
#                 return_attention_mask=True
#             )
            #audio_outputs = self.wavlm(input_values=audio_inputs['input_values'], attention_mask=audio_inputs['attention_mask'])
            # 提取处理后的音频特征（形状为[batch_size, max_length]）
            new_sentence_l = []
            ...
            tokenized_inputs = self.tokenizer(sentence_l, truncation=True, is_split_into_words=True,
                                              padding='max_length', max_length=60, return_tensors='pt')
            tokenized_inputs_cap=self.tokenizer(cap_sentences, truncation=True,is_split_into_words=True, padding='max_length',max_length=30, return_tensors='pt')
            text_labels = []
            cross_labels = []
            for i, label in enumerate(label_l):
                word_ids = tokenized_inputs.word_ids(batch_index=i)  # Map tokens to their respective word.
            ...
            #tokenized_inputs["audio_input_values"] = torch.stack(audios,dim=0)
            #tokenized_inputs["audio_attention_mask"] = torch.stack(audios_attention,dim=0)
            tokenized_inputs['dependency_matrics'] = torch.stack(dependency_matrics,dim=0)
            if dataset_type == 'train':
                tokenized_inputs['image_labels']=torch.stack(image_labels,dim=0)
            #tokenized_inputs['region_features'] = torch.stack(region_features,dim=0)

            tokenized_inputs['noun_masks'] = torch.stack(noun_masks,dim=0)
            if self.finetune_task == 'clipc':
                tokenized_inputs["clip_input_ids"] = clip_tokenized_inputs["input_ids"]
            self.input[dataset_type] = tokenized_inputs
    
    def load_audio_with_resample(self,path, target_sr=16000):
        # 读取音频
    ...
    def create_noun_mask(self, image_id):
        # 构建矩阵文件路径（替换扩展名）
        image_pt = image_id.split('.')[0] + '.npy'
        noun_mask_path = os.path.join('../roberta_noun_masks', image_pt)
...
def main():
    ...
    if dataset_type == '2015':
        ...
        data_image_dir = '../../lqmasa/data/raw_images'
