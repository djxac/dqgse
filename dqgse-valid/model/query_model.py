...
class TextImageFusionModel(nn.Module):
    def __init__(self,device, bert_model_name,num_layers, num_queries, query_dim, embed_dim, num_heads, hidden_dim,entity_type_count,dropout=0.1):
        ...
        # 加载预训练的 BERT 模型
        self.tokenizer = RobertaTokenizer.from_pretrained('./roberta-base-cased')

        self.bert = RobertaModel.from_pretrained('./roberta-base-cased')
        # 定义可学习的查询向量
        ...
        #self.sentence_representations = torch.mean(outputs.last_hidden_state[:,1:-1,:].detach(),dim=1)
    ...
    def forward(self, text_hidden_states, text_attention_mask, vision_hidden_states, vision_attention_mask):
        ...
        for layer in self.layers:
            output_queries, text_queries, image_queries= layer(tgt,pos, text_hidden_states,
                                                        vision_hidden_states,text_attention_mask,vision_attention_mask)
            tgt=output_queries
...
class TextImageFusionLayer(nn.Module):
    def __init__(self, embed_dim, num_heads, hidden_dim):
        ...
        #self.audio_cross_attention = CrossAttention(embed_dim=embed_dim, num_heads=num_heads)
        self.sublayer_connect2 = SublayerConnection(size=embed_dim)

        self.layer_norm = nn.LayerNorm(embed_dim)

        # 可学习参数：top-k 数量（或直接设为超参）
        self.k = 8  # 或通过配置传入
    ...
    def compute_global_features(self, text_hidden_states, vision_hidden_states, text_attention_mask, vision_attention_mask):
        """
    ...
    def forward(self, tgt, pos, text_hidden_states, vision_hidden_states,
                text_attention_mask, vision_attention_mask):
        batch_size = text_hidden_states.size(0)

        # Step 1: 自注意力
        query_with_pos = self.with_pos_embed(tgt, pos)
        query_self = self.self_attention(
            query=query_with_pos,
            key=query_with_pos,
            value=tgt,
            attention_mask=torch.ones(batch_size, tgt.size(1), device=tgt.device)
        )
        query_self = self.sublayer_connect0(x=tgt, sublayer_x=query_self)

        # Step 2: 文本交叉注意力
        text_query = self.with_pos_embed(query_self, pos)
        text_cross_output_only = self.text_cross_attention(
            query=text_query,
        ...
        # Step 3: 图像交叉注意力
        image_cross_query = self.with_pos_embed(text_cross_output, pos)
        image_cross_output_only = self.image_cross_attention(
            query=image_cross_query,
            key=vision_hidden_states,
            value=vision_hidden_states,
            attention_mask=vision_attention_mask
        )
        # 融合文本和图像注意力输出
        fused_output = text_cross_output_only + image_cross_output_only
        image_cross_output = self.sublayer_connect2(x=query_self, sublayer_x=fused_output)

#         # Step 4: 计算全局特征
#         text_global, img_global = self.compute_global_features(
#             text_hidden_states, vision_hidden_states,
#             text_attention_mask, vision_attention_mask
#         )  # [B, D]

#         # Step 5: 计算每个查询的相关性分数
#         # image_cross_output: [B, N, D]
#         queries = image_cross_output  # [B, N, D]

#         # 分别计算与文本全局、图像全局的相似度
#         sim_text = F.cosine_similarity(queries, text_global.unsqueeze(1), dim=-1)  # [B, N]
#         sim_img = F.cosine_similarity(queries, img_global.unsqueeze(1), dim=-1)   # [B, N]

#         # 综合得分（可加权）
#         scores = sim_text + sim_img  # [B, N]

#         # Step 6: 可微 Top-k 选择
#         k = min(self.k, tgt.size(1))  # 防止 k > N
#         soft_mask, topk_indices = self.differentiable_topk(scores, k=k, temperature=0.1)  # [B, N], [B, k]

#         # Step 7: 使用 soft_mask 加权查询（软选择）
#         image_cross_output = queries * soft_mask.unsqueeze(-1)  # [B, N, D]，未选中的接近 0

        # （可选）如果你只想保留 top-k 向量（硬选择但保留梯度），可以：
        # 注意：直接索引会丢失未被选中的梯度，但 soft_mask 已保证所有查询都参与前向
        # 所以我们返回加权后的 queries 即可

        # Step 8: 返回结果
        return (
            image_cross_output,           # 原始融合输出 [B, N, D]
            text_cross_output_only,       # 文本注意力输出
            image_cross_output_only,      # 图像注意力输出
        )
...
class UnifiedWeightGating(nn.Module):
    def __init__(self, input_dim):
        ...
        # 使用预训练的RoBERTa作为文本编码器，这里简化为LayerNorm
        self.text_norm = nn.LayerNorm(input_dim)
        self.image_norm = nn.LayerNorm(input_dim)
        self.audio_norm = nn.LayerNorm(input_dim)

        # 门控机制参数
        self.Wf_t2v = nn.Linear(input_dim, input_dim)
        self.bf_t2v = nn.Parameter(torch.zeros(input_dim))
        self.Wf_t2a = nn.Linear(input_dim, input_dim)
        self.bf_t2a = nn.Parameter(torch.zeros(input_dim))

    def forward(self, text_representation, image_representation, audio_representation):
        batch_size, seq_len, dim = text_representation.shape

        # 标准化操作
        text_rep = self.text_norm(text_representation)  # (batch_size, seq_len, dim)
        image_rep = self.image_norm(image_representation)  # (batch_size, seq_len, dim)
        audio_rep = self.audio_norm(audio_representation)  # (batch_size, seq_len, dim)

        # 计算门控信号
        gate_t2v = torch.sigmoid(self.Wf_t2v(text_rep) + self.bf_t2v)  # (batch_size, seq_len, dim)
        gate_t2a = torch.sigmoid(self.Wf_t2a(text_rep) + self.bf_t2a)  # (batch_size, seq_len, dim)

        # 基于门控信号筛选并增强多模态特征
        enhanced_image = gate_t2v * image_rep   # 强调与文本相关的图像特征
        enhanced_audio = gate_t2a * audio_rep  # 强调与文本相关的音频特征

        # 融合三个模态
        fused_representation = text_rep + enhanced_image + enhanced_audio
...
class SubTaskImg_f(nn.Module):
    ...
    def forward(self,img_hidden_states,token_mask,image_hidden_states_g,image_mask):
        img_output=self.multihead_attn(img_hidden_states,img_hidden_states,img_hidden_states,token_mask)
        img_output=self.sublayer_connect1(x=img_hidden_states,sublayer_x=img_output)
        img_output_g=self.multihead_attn(img_output,image_hidden_states_g,image_hidden_states_g,image_mask)
        img_output_g=self.sublayer_connect3(x=img_output,sublayer_x=img_output_g)
        img_output_feed=self.feed_forward(img_output_g)
        img_hidden=self.sublayer_connect2(x=img_output_g,sublayer_x=img_output_feed)
        return img_hidden
