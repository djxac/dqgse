...
from model.query_model import TextImageFusionModel, CLIPContrastiveLoss, TextImageFusion, TextOnlyMultiChannelGAT, \
    FeedForward, SublayerConnection, CrossAttention, ImageOnlyHypergraphConv


class DTCAModel_valid(nn.Module):
    def __init__(self,device,config1,config2,text_num_labels,alpha,beta,text_model_name="roberta",image_model_name='vit'):
        ...
        self.robert_e = RobertaModel.from_pretrained('./roberta-base-cased')
        embed_dim=768
        num_heads=8
        ...
        self.cross_classifier = nn.Sequential(
            nn.Linear(config1.hidden_size,3),
#             nn.ReLU(),
#             nn.Linear(512, 3)  # 输出 3 类
        )
...
#         self.layer_norm=LayerNorm(768)
    ...
    def forward(self,
                ...
                head_mask=None,
                cross_labels=None,
                return_dict=None,dependency_matrics=None,noun_masks=None,entity_input_ids=None,entity_attention_mask=None):
        ...
        entity_outputs=self.roberta(input_ids=entity_input_ids,attention_mask=entity_attention_mask)
        entity_last_hidden_states=entity_outputs["last_hidden_state"]
        ...
        text_cross_output_only=self.text_cross_attention(query=entity_last_hidden_states,key=text_last_hidden_states,value=text_last_hidden_states,attention_mask=attention_mask)
        text_cross_output = self.sublayer_connect1(x=entity_last_hidden_states, sublayer_x=text_cross_output_only)
        
        #通过交叉注意力层融合查询向量和视觉隐层状态
        image_cross_output_only= self.image_cross_attention(query=entity_last_hidden_states, key=image_last_hidden_states,
                                                 value=image_last_hidden_states, attention_mask=image_attention_mask)
        image_cross_output = self.sublayer_connect2(x=entity_last_hidden_states, sublayer_x=image_cross_output_only+text_cross_output_only)
        #cross_output=torch.cat([text_cross_output,image_cross_output],dim=-1)
        #image_cross_output=self.dropout(image_cross_output)
        cls_vector = image_cross_output[:,0]
        #cls_vector = self.dropout(cls_vector)
        # 通过分类层得到 logits
        logits = self.cross_classifier(cls_vector)  # shape: (batch_size, 3)

        # 如果提供了 cross_labels，则计算损失
        loss = None
