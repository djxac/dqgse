...
from transformers import RobertaModel, BertModel, AlbertModel, ElectraModel, ViTModel, SwinModel, DeiTModel, \
    ConvNextModel, CLIPVisionModel,WavLMModel, Wav2Vec2FeatureExtractor

from model.query_model import TextImageFusionModel, CLIPContrastiveLoss, TextImageFusion, TextOnlyMultiChannelGAT, \
    SubTaskAudio, SubTaskImg_f, FeedForward, SublayerConnection, CrossAttention,ImageOnlyHypergraphConv
...
class DTCAModel(nn.Module):
    def __init__(self,device,config1,config2,text_num_labels,alpha,beta,text_model_name="roberta",image_model_name='vit'):
        ...
        #self.wavlm = WavLMModel.from_pretrained('../WavLM')
#         for param in self.wavlm.parameters():
#                 param.requires_grad = False
        #self.projecter=SubTaskAudio(768,2048,8)
        #self.reshape=ReshapeSequence(440,60,768)
        #self.audio_cross_attention = CrossAttention(embed_dim=768, num_heads=8)
        #self.sublayer_connect_a = SublayerConnection(size=768)
        ...
        self.hypergraph=ImageOnlyHypergraphConv(768,768)
        
#         self.img_dropout = nn.Dropout(0.1)
#         self.classifier2=nn.Sequential(
#             nn.Linear(config1.hidden_size,2089)
#         )
#        # self.linear_loss=nn.HuberLoss(delta=1.0)
#         self.klloss = nn.KLDivLoss(reduction= 'batchmean')
    ...
    def forward(self,
                ...
                head_mask=None,
                cross_labels=None,
                return_dict=None,dependency_matrics=None,noun_masks=None,audio_input_values=None,audio_attention_mask=None):
        ...
        # cap_outputs=self.roberta(input_ids=cap_input_ids,attention_mask=cap_attention_mask)
        # cap_last_hidden_states=cap_outputs["last_hidden_state"]
        text_last_hidden_states = text_outputs["last_hidden_state"]

        image_last_hidden_states = image_outputs["last_hidden_state"]
        image_last_hidden_states=self.hypergraph(image_last_hidden_states)
#         audio_outputs = self.wavlm(audio_input_values, attention_mask=audio_attention_mask)
#         audio_last_hidden_states=audio_outputs["last_hidden_state"]

        #audio_attention_mask2=torch.ones(audio_last_hidden_states.size(0),audio_last_hidden_states.size(1), dtype=torch.long,device=audio_last_hidden_states.device)
        #audio_last_hidden_states=self.projecter(audio_input_values,audio_attention_mask)
        #audio_last_hidden_states=self.reshape(audio_last_hidden_states)
        ...
        #audio_mask=torch.ones(audio_last_hidden_states.size(0),audio_last_hidden_states.size(1), dtype=torch.long,device=audio_last_hidden_states.device)
        #tgt,text_queries,image_queries= self.query_model(text_last_hidden_states,attention_mask,image_last_hidden_states,image_attention_mask)
        #contract_loss=self.contract_loss(image_queries,text_queries)
        #contract_loss_ta=self.contract_loss(audio_queries,text_queries)
...
#         noun_hidden=text_hidden_states_n.sum(dim=1)/(torch.sum(noun_masks,dim=1,keepdim=True) + 1e-8)
        ...
        #noun_loss2=self.cosine_similarity_loss(torch.mean(image_last_hidden_states,dim=1),noun_hidden)
        ...
        #image_text_cross_attention,_=self.image_text_cross(text_hidden_states_syn,combined_context,combined_context,combined_mask)
        #image_text_cross_feed=self.feed_forward(image_text_cross_attention)
        #image_text_output=self.sublayer(x=image_text_cross_attention,sublayer_x=image_text_cross_feed)
        image_text_cross_attention, _ = self.image_text_cross(text_hidden_states_syn,image_last_hidden_states, image_last_hidden_states)


        cross_logits = self.classifier0(image_text_cross_attention)
        #cross_logits = self.classifier0(image_text_output)
        mask = (labels != -100)
        mask[:,0] = 1
...
#         word_region_align_loss = ot_dist.mean()
        # 转化为概率分布
        # text_probs = F.softmax(F.normalize(text_queries,p=2,dim=1), dim=-1)
        # image_probs = F.softmax(F.normalize(image_queries,p=2,dim=1), dim=-1)
        # #
        # # # 计算KL散度 提取完特征后对齐
        # kl_div_loss = kl_divergence_loss(text_probs, image_probs)

        # text_loss
        sequence_output1 = self.dropout(text_last_hidden_states)
        text_token_logits = self.classifier1(sequence_output1)
        #
        # sequence_output2 = self.dropout(text_hidden_states_syn)
        # text_syn_logits=self.classifier2(sequence_output2)
        #img_tag_loss=None
#         if image_labels is not None:
#             image_last_hidden_states_mean=self.img_dropout(torch.mean(image_last_hidden_states,dim=1))
#             img_tag=self.classifier2(image_last_hidden_states_mean).squeeze()
#             img_tag_softmax=F.softmax(img_tag,dim=-1)
        
#             img_tag_loss=self.klloss(img_tag_softmax.log(),image_labels)
...
#         if image_labels is not None:
#             loss = cross_crf_loss + 0.3 * text_loss+contract_loss*self.alpha +noun_loss*self.beta+img_tag_loss
#         else:
#             loss = cross_crf_loss + 0.3 * text_loss + contract_loss * self.alpha + noun_loss_2 * self.beta
        loss = cross_crf_loss + 0.3 * text_loss
        #loss = cross_crf_loss + 0.3 * text_loss + contract_loss*self.alpha +noun_loss2
